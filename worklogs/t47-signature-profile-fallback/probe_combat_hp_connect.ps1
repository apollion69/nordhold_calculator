...
$path = Join-Path $artifactDir "memory_calibration_candidates_combat_hp_autoload.json"
$payload = @{
...
$connect | ConvertTo-Json -Depth 12 | Set-Content -Path (Join-Path $artifactDir "live_connect_combat_hp.json") -Encoding UTF8
$status | ConvertTo-Json -Depth 12 | Set-Content -Path (Join-Path $artifactDir "live_status_combat_hp.json") -Encoding UTF8
$snapshot | ConvertTo-Json -Depth 12 | Set-Content -Path (Join-Path $artifactDir "live_snapshot_combat_hp.json") -Encoding UTF8
[PSCustomObject]@{
  mode = $status.mode
