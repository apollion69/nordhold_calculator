...
| P-018 Final 1800s acceptance (live process required) | [~] | codex | 2026-03-01T20:10:00+04:00 | 2026-03-01T20:25:00+04:00 | `runtime/logs/nordhold-live-soak-20260301_230456.summary.json` | Зависит от живого процесса `NordHold.exe` + стабильного autoload | Дождаться live-окна и выполнить `run_nordhold_live_soak.ps1 -DurationS 1800` с `-CandidateTtlHours 24`; ожидаемый PASS: `candidate_set_stale=false`, `final_stop_reason` пуст/`completed=true` |
| P-019 Pass readiness + candidate recovery loop | [ ] | codex | 2026-03-01T20:30:00+04:00 |  | `runtime/logs/nordhold-live-soak-<ts>.summary.json` / `worklogs/t56-memory-soak/<run_id>/` | mass `winerr=299` на ранних попытках; требуется подтвержденный fresh `memory_calibration_candidates_autoload.json` | Выполнить 2–3 цикла `refresh→promote→resolve` до появления стабильного `winner_candidate_id`; затем запуск `DurationS 30` и `DurationS 1800` |
...
- Критический blocker для `P-010`: не проведён LIVE 1800s PASS из-за массового `winerr=299` во всех кандидатах. Свежий `DurationS 1800` прогон `runtime/logs/nordhold-live-soak-20260301_230456.summary.json` остановлен через `autoconnect_candidate_set_stale` (`candidate_set_stale_reason=mass_connect_transient_299`, `transient_299_share=1`, `winner_candidate_id=""`).
